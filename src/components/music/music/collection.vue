...
<div :class="['px-4 w-full rounded-[22px] bg-white']">
  <van-list
    ...
    finished-text="没有更多了">
    <div>
      <div class="relative flex items-center" v-for="(item,index) in listNow" @click="toCollectViewHandle(index)" >
        <img :src="item.picUrl" alt="" class="mr-2 w-[36px] rounded-sm my-2" >
        <div class="my-2">
        ...
        <img src="../../../assets/cancel.svg" alt="" class="w-[22px] absolute right-0" @click.stop="cancelCollection(index)">
      </div>
    </div>
  </van-list>
</div>
