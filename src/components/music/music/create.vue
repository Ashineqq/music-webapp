...
const { newCreateVisible, inputNameList, songTemp,collectToCreateVisible } = storeToRefs(newCreate)

//进行存储
const collectToCreateHandle = (albumIndex: number) => {
  if (songTemp.value.al.name === '') { return }
...
<div :class="['px-4 w-full rounded-[22px] bg-white']">
  <van-list
    ...
    finished-text="没有更多了">
    <div>
      <div class="relative flex items-center" v-for="(item,index) in inputNameList" >
        <img src="../../../assets/image.png" alt="" class="mr-2 w-[36px] rounded-sm my-2" @click="toCreatelistHandle(item.song.length,index)">
        ...
        <img src="../../../assets/cancel.svg" alt="" class="w-[22px] absolute right-0" @click="cancleAlbumHandle(index)">
      </div>
      <div class="flex items-center"  @click="newCreateVisible = true">
        <div class=" bg-black/10 w-[36px] h-[36px] flex items-center justify-center mr-2">
          <img src="../../../assets/newcreate.svg" alt="" class="w-[24px] rounded-sm">
        </div>
