...
<template>
<div class="w-screen">
  <div v-if="newCreateVisible" class="flex flex-col fixed rounded-t-[16px] bottom-0 z-50 w-screen px-4 bg-[#f1e9e9] h-[30vh] py-2">
    <!-- 取消完成功能键 -->
    <div class="flex items-center justify-between text-black text-[12px] my-2 flex-1">
    ...
    <div class="flex items-center justify-center  w-full flex-5 -mt-10">
      <input v-model="inputVal" type="text" placeholder="输入新建歌单标题" class="p-2 border rounded-[10px] w-full">
    </div>
  </div>
</div>
</template>