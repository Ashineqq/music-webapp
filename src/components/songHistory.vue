...
<template>
  <body>
    <div
      class="fixed bottom-0 w-full flex flex-col z-20 h-[75vh] rounded-[20px] pt-4 bg-white"
      v-if="visable"
      ref="componentRef"
    >
      <!-- title -->
      <div class="h-1/8 px-4 relative">
        ...
        <img
          ...
          alt=""
          class="w-[24px] absolute right-4"
          @click="cleanListHandle"
        />
      </div>
      <!-- 当前播放列表 -->
      <div class="overflow-y-auto h-7/8">
        ...
        >
          <div
            :class="[
              'flex items-center justify-between py-2 px-4 text-[#67666a]',
          ...
          >
            ...
            <img
              ...
              alt=""
              class="w-[24px]"
              @click.stop="cleanSigleHandle(item)"
            />
          </div>
        </van-list>
      </div>
