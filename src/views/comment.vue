  ...
  <div :class="['flex flex-col h-screen w-full', { 'bg-black/50': visible }]">
    ...
    <div class="flex items-center h-1/12 mx-4">
      <img
        ...
        alt=""
        class="w-[36px] rounded-full mr-1"
      />
    ...
    <div class="px-4 w-full overflow-y-auto h-18/24">
      ...
      >
        <div
          class="flex flex-col py-4 border-b-1 border-[#f6f6f6]"
          v-for="item in hComments"
          :key="item.commentId"
        >
          <div class="flex items-center relative">
            <img
              ...
              alt=""
              class="w-[36px] rounded-full mr-2"
            />
            <div>
              <p class="text-[#808080] text-[12px]">{{ item.user.nickname }}</p>
              <div class="flex items-center">
                <p class="mr-1 text-[10px] text-[#d0d0d0]">
                  {{ item.timeStr }}
                </p>
                <p class="text-[10px] text-[#d0d0d0]">
                  {{ item.ipLocation.location }}
                </p>
              </div>
              <!-- 点赞 -->
              <div class="absolute right-0 top-2 flex items-center">
                <p class="text-[#d0d0d0] text-[12px] mr-[1px]">
                  {{ item.likedCount }}
                </p>
          ...
          <div class="flex flex-col ml-[42px] pt-1">
            <p class="text-[#3f3f43] text-[12px] font-sans">
              {{ item.content }}
            </p>
          </div>
          <!-- 回复 -->
    ...
    <!-- 发评论 -->
    <div
      class="fixed bg-white z-10 bottom-0 flex items-center w-full px-4 h-[60px]"
    >
      <input
        v-model="sendInput"
        type="text"
        placeholder="千头万绪，落笔汇成评论一句"
        class="flex-1 mr-2 bg-[#f2f3f5] rounded-full p-2"
      />
      <div class="w-20px text-[#cdcdcf] text-[14px]" @click="sendCommentHandle">
        发送
      </div>
    </div>
  </div>
