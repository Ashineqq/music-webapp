...
<div class="h-[92vh] w-full flex-col flex overflow-y-auto">
  ...
  <div class="relative h-1/2 w-full" id="img" >
    ...
    <div class="flex items-center justify-center relative h-3/12">
      <img src="../../assets/image.png" alt="" class="w-[58px] rounded-full">
    </div>
    <!-- 名称等信息 -->
    ...
    <div class="flex items-center mx-4 text-[10px]">
      <div class="flex items-center justify-center p-2 mr-2 bg-white/25 rounded-sm h-[30px] flex-1">
      ...
      <div class="flex items-center justify-center p-2 bg-white/25 rounded-sm h-[30px] flex-1">
  ...
  <!-- 下半部分 -->
  <div class="relative h-12/24 w-full rounded-[10px] bg-white z-10 -mt-5 pt-2">
    <!-- sticky区域 -->
    <div class="sticky top-0">
      <!-- 大切换栏 -->
      <div class="flex items-center justify-evenly mb-2 ">
        <div class="flex flex-col items-center" @click="parentClickedAHandle">
          <p :class="['text-[15px] font-bold',{'text-black':parentClickedA}]">音乐</p>
          <div class=" border-1 w-[16px] border-[#fb2a59]" v-if="parentClickedA"></div>
        </div>
        <div class="flex flex-col items-center" @click="parentClickedBHandle">
          <p :class="['text-[15px] font-bold',{'text-black':parentClickedB}]">播客</p>
          <div class=" border-1 w-[16px] border-[#fb2a59]" v-if="parentClickedB"></div>
        </div>
        <div class="flex flex-col items-center" @click="parentClickedCHandle">
          <p :class="['text-[15px] font-bold',{'text-black':parentClickedC}]">动态</p>
          <div class=" border-1 w-[16px] border-[#fb2a59]" v-if="parentClickedC"></div>
        </div>
      </div>
      <!-- 小切换栏 -->
